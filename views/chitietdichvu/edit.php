...
<div class="ctdv-form-wrap">
    <div class="ctdv-form-title">Sửa Chi tiết Dịch vụ</div>
    <form class="ctdv-form" id="ctdv-edit-form" autocomplete="off">
        <label>Mã ĐL:
            <input type="text" name="madl" required readonly>
        </label>
        <label>Mã DV:
            <input type="text" name="madv" required placeholder="Nhập mã DV mới">
        </label>
        <div class="ctdv-form-btns">
            <button type="submit" class="ctdv-btn">Lưu thay đổi</button>
            <button type="button" class="ctdv-btn ctdv-back">Quay lại</button>
        </div>
        <div class="ctdv-form-msg" id="ctdv-edit-msg"></div>
    </form>
</div>
<script>
document.getElementById('ctdv-edit-form').onsubmit = function(e) {
    e.preventDefault();
    const madl = this.madl.value.trim();
    const madv = this.madv.value.trim();
    const msg = document.getElementById('ctdv-edit-msg');
    msg.textContent = '';
